...
--Olhar o novo Tipo Algébrico Tarefas presente em Tipos.hs
--Função que imprime o relatório das tarefas na tela do usuário
--Forma de entrada e saída, usos de let para utilizar funções puras
...
--Imprime a frequencia de cada tag de forma unitária a partir de uma lista de tarefas                   
...
--Imprime a partir de ua lista, quais tarefas possuem uma TAG digitada pelo usuário
...
--Utiliza função auxiliar descrita abaixo        
...
--Faz a impressão de fato dos dias restantes (utiliza lib Data.time)
...
--Impressão de Tarefas atrasadas
...
--Imrpime TODAS tarefas existentes na lista          
...
--adiciona a lista pre-definida em Funcoes.hs
